...
def evaluate_sr_seg_on_test_set(model, test_loader, classes, CLASSES_TO_RGB, image_dir=None, wandb_setup=True, num_samples=5):
    ...
    with torch.no_grad():
        ...
        for batch_index, (inputs, masks, lr_images, *_) in enumerate(progress_bar):
            ...
            plot_predictions(lr_images, seg_masks, masks, classes=classes, epoch=1, num_samples='', sr_images=sr_images, groundtruths=inputs, image_dir=image_dir, batch_index=batch_index, batch_size=batch_size, CLASSES_TO_RGB=CLASSES_TO_RGB)
