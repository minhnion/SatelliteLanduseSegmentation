...
from utils.non  import NONLocalBlock2D
from utils.tools import extract_image_patches,\
    reduce_mean, reduce_sum, same_padding, reverse_patches

from utils.transformer import drop_path, DropPath, PatchEmbed, Mlp,MLABlock
...
class ESRT(nn.Module):
    def __init__(self, n_blocks=1, n_channels=3, upscale=4, conv=common.default_conv, encoder_only = False):
        ...
        self.up = nn.Sequential(common.Upsampler(conv,scale,n_feats,act=False),
                          BasicConv(n_feats, n_channels,kernel_size,1,1))
    ...
    def freeze_layers(self, freeze_head, freeze_body, freeze_tail):
        """ Freeze layers while training """
        if freeze_head:
