...
#         #     print(f"[ERROR] Shape mismatch! Image: {image.shape}, Mask: {mask.shape}, Index: {idx}")
#         #     # Nếu có thể truy xuất tên file thì in ra luôn
#         #     if hasattr(self, 'images'):
#         #         print(f"Image file: {self.images[idx]}")
#         #     if hasattr(self, 'masks'):
#         #         print(f"Mask file: {self.masks[idx]}")
#         #     raise AssertionError("Image and Mask shape mismatch")
...
# class LandCoverDataset(Dataset):
#     def __init__(self, root_dir, transforms=None, patch_size=16, scale_factor=2,
#                  num_tiles=4, file_paths=None, mask_scale=None):
#         self.root_dir = root_dir
#         self.transform = transforms
#         self.image_paths = []
#         self.mask_paths = []
...
#     def __len__(self):
#         return len(self.image_paths) * (self.num_tiles ** 2) if self.num_tiles else len(self.image_paths)
...
#         # --- Load image ---
#         if image_path.endswith(".tif"):
#             image = open_tif_image(image_path)  # (H,W,C)
#         else:
#             image = np.array(Image.open(image_path).convert('RGB')) / 255.0

#         # --- Load mask ---
#         mask = np.array(Image.open(mask_path).convert('RGB'))
#         # Align mask size if mismatch
#         if image.shape[:2] != mask.shape[:2]:
...
#         # --- Downscale for SCNet ---
#         if self.scale_factor:
#             # image is a torch.Tensor after transform
#             lr_image = downscale_image(image.numpy(), self.scale_factor)
#             fractional_alpha = compute_fraction_image(lr_image)
#             return image, mask, lr_image, fractional_alpha, image_path, mask_path
...
# class LandCoverHRDataset(Dataset):
#     def __init__(self, root_dir, transforms=None, scale_factor=16,
#                  num_tiles=4, file_paths=None):
#         self.root_dir = root_dir
#         self.transform = transforms
#         self.image_paths, self.lr_image_paths, self.mask_paths = [], [], []
#         self.scale_factor = scale_factor
#         self.num_tiles = num_tiles
...
#         for file_name in file_paths:
#             if file_name.endswith(('lr_sat.jpg', 'lr_sat.tif')):
#                 lr_image_path = os.path.join(root_dir, file_name)
#                 self.lr_image_paths.append(lr_image_path)
...
#                 if image_path.endswith('_sat.jpg'):
#                     mask_path = image_path.replace('_sat.jpg', '_mask.png')
#                 elif image_path.endswith('_sat.tif'):
#                     mask_path = image_path.replace('_sat.tif', '_mask.png')
#                 self.mask_paths.append(mask_path)

#         assert len(self.image_paths) == len(self.mask_paths) == len(self.lr_image_paths)
...
#         if image_path.endswith(".tif"):
#             image = open_tif_image(image_path)
#             # Only swap bands if ≥3 channels (Sentinel-2 RGB)
#             if image.shape[2] >= 3:
#                 image = image[:, :, [2, 1, 0]]
#         else:
#             image = np.array(Image.open(image_path).convert('RGB')) / 255.0
...
#             lr_image = open_tif_image(lr_image_path)
#         else:
#             lr_image = np.array(Image.open(lr_image_path).convert('RGB')) / 255.0
...
#         # Check LR scaling consistency
#         h_lr, w_lr = lr_image.shape[:2]
#         h_img, w_img = image.shape[:2]
#         if h_lr != h_img // self.scale_factor or w_lr != w_img // self.scale_factor:
#             # resize lr_image if mismatch
#             lr_image = cv2.resize(lr_image, (w_img // self.scale_factor, h_img // self.scale_factor),
...
class LandCoverDataset(Dataset):
    def __init__(self, root_dir, transforms=None, patch_size=16, scale_factor=None, num_tiles=4, file_paths=None, mask_scale=None):
        self.root_dir = root_dir
        self.transform = transforms
        self.image_paths = []
        self.mask_paths = []
        self.patch_size = patch_size  # Patch size for fractional mask
        self.scale_factor = scale_factor  # Downscaling factor (can be None)
        self.num_tiles = num_tiles  # Number of tiles per row/column
        self.mask_scale = mask_scale  # Scale factor for masks
        ...
        for file_name in file_paths:
            if file_name.endswith(('.jpg', '.tif', '.tiff')):
                image_path = os.path.join(root_dir, file_name)
                self.image_paths.append(image_path)
    ...
    def __getitem__(self, idx):
        ...
        # --- Load image ---
        if image_path.endswith(".tif"):
            image = open_tif_image(image_path)  # Sentinel-1: shape (H, W, C)
        else:
            image = np.array(Image.open(image_path).convert("RGB")) / 255.0  # Normalize [0, 1]

        # --- Load mask as RGB (needed for rgb_to_class later) ---
        mask = np.array(Image.open(mask_path).convert("RGB"))
        ...
        assert h_mask == h_image * scale and w_mask == w_image * scale, \
            f"Mismatch: image {image.shape}, mask {mask.shape}, scale {scale}"

        # --- Handle downscaling safely ---
        if self.scale_factor:
            lr_image = downscale_image(image.numpy(), self.scale_factor)
            fractional_alpha = compute_fraction_image(lr_image)
        else:
            lr_image = torch.zeros(1)             # placeholder instead of None
            fractional_alpha = torch.zeros(1)     # placeholder instead of None
...
class LandCoverHRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load HR image
        if image_path.endswith(".tif"):
            image = open_tif_image(image_path)  # Sentinel-1
        else:
            image = np.array(Image.open(image_path).convert("RGB")) / 255.0
        ...
        assert h_lr_image == h_image // self.scale_factor and w_lr_image == w_image // self.scale_factor, \
            f"LR shape mismatch: HR {image.shape}, LR {lr_image.shape}"

        assert image.shape[:2] == mask.shape[:2], \
            f"Image and mask mismatch: {image.shape}, {mask.shape}"
