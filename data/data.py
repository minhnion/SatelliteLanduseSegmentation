...
from utils.image_utils import open_tif_image, split_image, downscale_image
import numpy as np
import torch
import torch.nn as nn

class LandCoverDataset(Dataset):
    def __init__(self, root_dir, transforms=None, num_tiles=6, rgb_only=False, mask_scale=None, scale_factor=None):
        self.root_dir = root_dir
        self.transform = transforms
        self.image_paths, self.mask_paths = [], []
        self.num_tiles = num_tiles if num_tiles else 1
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image, mask = self.transform(image, mask, self.mask_scale)
        ...
        if self.scale_factor:
            ...
            return image, mask, lr_image, image_path, mask_path
